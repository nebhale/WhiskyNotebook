    ...
    override func spec() {
        describe("Configuration") {
            it("initializes all properties to values") {
                ...
                expect(configuration.level).toNot(beNil())
                expect(configuration.format).toNot(beNil())
            }
            
            it("bases equality on name") {
                expect(Configuration(name: "test-name")).to(equal(Configuration(name: "test-name")))
                expect(Configuration(name: "test-name")).toNot(equal(Configuration(name: "another-test-name")))
            }
